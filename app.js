...
function applyRoleAccess() {
  ...
  // Communication tab: hide Settings sub-tab for non-admins
  document.querySelectorAll("[data-comm-tab]").forEach((tab) => {
...
function renderCommunicationView() {
  ...
  document.querySelectorAll("[data-comm-tab]").forEach(tab => {
    const isSettings = tab.dataset.commTab === "settings";
    tab.classList.toggle("hidden", !isAdmin && isSettings);
  ...
  // Hide template sections for non-admins
  if (!isAdmin) {
    document.querySelectorAll("#commSmsTemplatesBody, #commSmsAddRange, #commSmsSaveBtn, #commSmsSavedBadge").forEach(el => el?.classList.add("hidden"));
    document.querySelectorAll("#commWaTemplatesBody, #commWaAddRange, #commWaSaveBtn, #commWaSavedBadge").forEach(el => el?.classList.add("hidden"));
    document.querySelectorAll("[data-comm-section] .section-head").forEach(el => {
      if (el.querySelector("h2")?.textContent?.includes("Template")) el.classList.add("hidden");
    });
  }
