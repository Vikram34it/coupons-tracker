...
function bindEvents() {
  ...
  const scanBtn = document.getElementById("scanCouponBtn");
  if (scanBtn) {
    scanBtn.addEventListener("click", startCouponScanner);
  }
...
function renderEntryList() {
  ...
  const scanBtn = document.getElementById("scanCouponBtn");
  if (scanBtn) scanBtn.classList.toggle("hidden", activeDevoteeTab !== "pending");
...
// ═══════════════════════════════════════════════
// 📷 CAMERA OCR SCANNER FOR COUPON AUTO-FILL
// ═══════════════════════════════════════════════

let _ocrLoaded = false;

function ensureOcrLoaded() {
  if (typeof Tesseract !== "undefined") { _ocrLoaded = true; return Promise.resolve(); }
  if (_ocrLoaded) return Promise.resolve();
  return loadScript("https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js")
    .then(() => { _ocrLoaded = true; })
    .catch(() => {});
}

let cameraStream = null;
let scannerEscapeHandler = null;
...
function stopCamera() {
  ...
  const statusEl = document.getElementById("ocrScannerStatus");
  if (statusEl) statusEl.textContent = "";
  if (scannerEscapeHandler) {
    document.removeEventListener("keydown", scannerEscapeHandler);
    scannerEscapeHandler = null;
  }
}

function startCouponScanner() {
  if (activeDevoteeTab !== "pending") {
    showToast("Switch to Pending Coupons tab to scan");
    return;
  }
  ...
  ensureOcrLoaded().then(() => {
    if (typeof Tesseract === "undefined") {
      showToast("OCR library failed to load. Check internet.");
      return;
    }

    let overlay = document.getElementById("ocrScannerOverlay");
    if (!overlay) {
      overlay = document.createElement("div");
      overlay.id = "ocrScannerOverlay";
      overlay.className = "modal-overlay";
      overlay.innerHTML = `
        <div class="modal-card" style="max-width:420px">
          <h3>📷 Scan Handwritten Coupon</h3>
          <p class="hint" style="margin-bottom:10px">Position the coupon in frame and tap <strong>Capture</strong>.</p>
          <div style="position:relative;width:100%;aspect-ratio:1;max-height:320px;overflow:hidden;border-radius:8px;background:#000">
            <video id="ocrViewfinder" autoplay playsinline style="width:100%;height:100%;object-fit:cover"></video>
            <canvas id="ocrCaptureCanvas" style="display:none"></canvas>
          </div>
          <div class="inline-fields" style="margin-top:10px">
            <button type="button" id="ocrCaptureBtn" class="primary">📸 Capture</button>
            <button type="button" id="ocrScannerClose" class="ghost">Cancel</button>
          </div>
          <p id="ocrScannerStatus" class="hint" style="margin-top:8px;text-align:center">Loading camera...</p>
          <div id="ocrPreviewArea" style="display:none;margin-top:8px"></div>
        </div>`;
      document.body.appendChild(overlay);

      overlay.querySelector("#ocrScannerClose").addEventListener("click", stopCamera);
      overlay.querySelector("#ocrCaptureBtn").addEventListener("click", captureAndOcr);
      overlay.addEventListener("click", (e) => {
        if (e.target === overlay) stopCamera();
      });
      document.addEventListener("keydown", scannerEscapeHandler = (e) => {
        if (e.key === "Escape" && !overlay.classList.contains("hidden")) stopCamera();
      });
    }

    overlay.classList.remove("hidden");
    const viewfinder = document.getElementById("ocrViewfinder");
    const statusEl = document.getElementById("ocrScannerStatus");
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment", width: { ideal: 1280 }, height: { ideal: 720 } } })
      .then((stream) => {
        ...
        statusEl.textContent = "Tap Capture when ready";
      })
      .catch(() => {
...
function captureAndOcr() {
  ...
  canvas.width = viewfinder.videoWidth || 1280;
  canvas.height = viewfinder.videoHeight || 720;
  ...
  showToast("Processing handwritten text...");
  ...
  Tesseract.recognize(canvas.toDataURL("image/jpeg", 0.8), "eng", {
    ...
    statusEl.textContent = "✅ Text read. Parsing fields...";
    parseAndFillOcr(text);
  }).catch(() => {
    statusEl.textContent = "Text recognition failed. Try again.";
    showToast("OCR failed. Ensure good lighting and clear handwriting.");
  });
}

function parseAndFillOcr(rawText) {
  ...
  const parsed = { name: "", contact: "", amount: "", seva: "" };

  // Extract 10-digit phone number
  const phoneMatch = text.match(/\b(\d{10})\b/);
  if (phoneMatch) {
    parsed.contact = phoneMatch[1];
  }

  // Extract amount (₹ or Rs prefix, or standalone 3+ digit number)
  const amountMatch = text.match(/(?:₹|Rs\.?\s*)?(\d{3,})/i);
  if (amountMatch) {
    parsed.amount = amountMatch[1];
  }

  // Match known seva types
  const textLower = text.toLowerCase();
  const matchedSeva = SEVA_TYPES.find(s => textLower.includes(s.toLowerCase()));
  if (matchedSeva) {
    parsed.seva = matchedSeva;
  }

  // For name: take the first line that isn't a phone, amount, or seva
  for (const line of lines) {
    const lineLower = line.toLowerCase();
    const isPhone = /\b\d{10}\b/.test(line);
    const isAmount = /(?:₹|Rs\.?\s*)?\d{3,}/i.test(line);
    const isSeva = SEVA_TYPES.some(s => lineLower.includes(s.toLowerCase()));
    const isShort = line.length < 3;
    if (!isPhone && !isAmount && !isSeva && !isShort) {
      parsed.name = line.replace(/^[#\d\s]+/, "").trim();
      break;
    }
  }
  ...
  if (!parsed.name && lines.length > 0) {
    parsed.name = lines[0].replace(/^[#\d\s]+/, "").trim();
  }
  ...
  if (overlay) {
    overlay.innerHTML = `
      <div class="modal-card" style="max-width:420px">
        ...
        <p class="hint" style="margin-bottom:10px">Review the recognized text below and correct if needed.</p>
        <div style="background:var(--bg-alt, #f5f5f5);padding:12px;border-radius:8px;margin-bottom:12px;font-size:13px;max-height:150px;overflow-y:auto;white-space:pre-wrap">${escapeHtml(rawText)}</div>
        <label>Coupon # <input id="ocrCouponNumber" type="number" min="1" max="${state.coupons.length}" style="width:100px" placeholder="Number"></label>
        <label>Buyer Name <input id="ocrBuyerName" value="${escapeAttr(parsed.name)}" placeholder="Name"></label>
        <label>Contact <input id="ocrBuyerContact" type="tel" value="${escapeAttr(parsed.contact)}" placeholder="10-digit phone"></label>
        <label>Amount <input id="ocrAmount" type="number" min="0" value="${escapeAttr(parsed.amount)}" placeholder="Amount"></label>
        <label>Seva Type
          <select id="ocrSevaType">
            <option value="">Select Seva</option>
            ${SEVA_TYPES.map(s => `<option value="${escapeAttr(s)}" ${parsed.seva === s ? "selected" : ""}>${escapeHtml(s)}</option>`).join("")}
          </select>
        </label>
        <div class="inline-fields" style="margin-top:12px">
          <button type="button" id="ocrFillBtn" class="primary">✅ Fill Coupon</button>
          <button type="button" id="ocrRetakeBtn" class="ghost">🔄 Retake</button>
          <button type="button" id="ocrDoneClose" class="ghost">Cancel</button>
        </div>
      </div>`;
    overlay.classList.remove("hidden");

    overlay.querySelector("#ocrFillBtn").addEventListener("click", () => {
      const couponNumber = Number(document.getElementById("ocrCouponNumber").value);
      if (!couponNumber || couponNumber < 1 || couponNumber > state.coupons.length) {
        showToast("Enter a valid coupon number");
        return;
      }
      const data = {
        buyerName: document.getElementById("ocrBuyerName").value.trim(),
        buyerContact: document.getElementById("ocrBuyerContact").value.trim(),
        amount: document.getElementById("ocrAmount").value.trim(),
        description: document.getElementById("ocrSevaType").value
      };
      overlay.classList.add("hidden");
      fillCouponFromScan(couponNumber, data);
    });

    overlay.querySelector("#ocrRetakeBtn").addEventListener("click", () => {
      overlay.classList.add("hidden");
      setTimeout(startCouponScanner, 300);
    });

    overlay.querySelector("#ocrDoneClose").addEventListener("click", () => {
      overlay.classList.add("hidden");
    });
...
function fillCouponFromScan(couponNumber, data) {
  const coupon = state.coupons[couponNumber - 1];
  if (!coupon || !coupon.devoteeId) {
    showToast(`Coupon #${couponNumber} is not assigned to any devotee`);
    return;
  }

  if (session?.role === "devotee" && coupon.devoteeId !== session.devoteeId) {
    showToast(`Coupon #${couponNumber} is not assigned to you`);
    return;
  }

  if (coupon.settled) {
    showToast(`Coupon #${couponNumber} is already settled`);
    return;
  }

  if (els.entryDevotee) {
    els.entryDevotee.value = coupon.devoteeId;
  }

  document.querySelectorAll("[data-devotee-tab]").forEach(t => t.classList.remove("active"));
  document.querySelector('[data-devotee-tab="pending"]')?.classList.add("active");
  activeDevoteeTab = "pending";

  // Apply scanned data to coupon
  if (data.buyerName) coupon.buyerName = data.buyerName;
  if (data.buyerContact) coupon.buyerContact = data.buyerContact;
  if (data.amount) coupon.amount = Number(data.amount);
  ...
  const card = document.querySelector(`[data-coupon-number="${couponNumber}"]`);
  if (card) {
  ...
  showToast(`Coupon #${couponNumber} updated from scan`);
}
